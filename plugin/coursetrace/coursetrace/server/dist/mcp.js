...
export function createMcpServer(store) {
    ...
    server.registerTool("get_current_class", {
        ...
        if (!snapshot)
            return textResult({ match: null, reason: "Android device has not uploaded a schedule snapshot" });
    ...
    server.registerTool("start_class_session", {
        ...
        if (!ownerId || !ownerType || !resolvedTitle)
            throw new Error("Course is ambiguous; ask the user to identify it");
        const session = await store.createSession({ ownerId, ownerType: ownerType, title: resolvedTitle });
        await store.enqueue("SESSION_STARTED", session);
        return textResult(session);
    });
    server.registerTool("append_learning_event", {
        ...
        if (!await store.session(session_id))
    ...
    server.registerTool("append_transcript_chunk", {
    ...
    server.registerTool("finish_class_session", {
    ...
    server.registerTool("query_learning_records", {
        ...
        const filtered = records.filter((record) => {
            if (owner_id && record.ownerId !== owner_id)
                return false;
            if (from && typeof record.timestamp === "string" && record.timestamp < from)
                return false;
            if (to && typeof record.timestamp === "string" && record.timestamp > to)
                return false;
            if (kinds?.length && typeof record.kind === "string" && !kinds.includes(record.kind))
                return false;
            return true;
        });
        return textResult({ records: filtered.slice(-500), snapshotUpdatedAt: (await store.snapshot())?.updatedAt ?? null });
    });
    server.registerTool("create_timetable_import_draft", {
    ...
    server.registerTool("save_next_material_prediction", {
        ...
        if (input.material_ids.some((id) => !known.has(id)))
            throw new Error("Prediction references an unknown material ID");
        const change = await store.enqueue("NEXT_MATERIAL_PREDICTION", input);
        return textResult({ saved: true, sequence: change.sequence });
    });
    server.registerTool("sync_status", { description: "Return paired-device snapshot age and queued-change count.", inputSchema: {} }, async () => textResult(await store.status()));
    return server;
}
